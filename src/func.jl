...
function SwitchTeams!(_SwissDraw, _teamA::String, _teamB::String,_round=missing)
    ...
    if size(games2Mod,1) == 1 
        ...
        premod = deepcopy(games2Mod[1])

        games2Mod[1].teamA = premod.teamB
        games2Mod[1].teamB = premod.teamA
...
"""
This function is the meat of the swiss draw. It takes a list of previous games, and the bye indicator, 
...
"""
function CreateNextRound!(_SwissDraw::SwissDraw; refresh=false)
    ...
    fdf = deepcopy(_SwissDraw.layout.fieldDF)
    bye = :middle
    ...
    df = rankings(_SwissDraw, allGames = allGamesBool)
    ...
    costMatchup = zeros(teamSz,teamSz) # mag ~10^1
    costSelfMatchup = zeros(teamSz,teamSz) # mag ~10^6
    costAnotherBye = zeros(teamSz,teamSz) # mag ~10^2
    costSelectBye = zeros(teamSz,teamSz) # mag ~10^1

    for i in 1:teamSz
        for j in 1:teamSz

            # This makes large matchups more costly
            costMatchup[i,j] = max(abs.(df.strength[i] - df.strength[j]))*10

            # # This stops teams from playing themself
            if i == j 
                costSelfMatchup[i,j] = 1000000
            end

            # This prevents teams that have had a bye from having another one
            if ((df.teamA[i]=="BYE" && df.byeA[j]==1 ) || (df.teamA[j]=="BYE" && df.byeA[i]==1 ))
                costAnotherBye[i,j] = 100
            end


            if bye == :middle 
                if df.teamA[i]=="BYE" 
                    costSelectBye[i,j] = abs.(size(df.rank,1)/2 .- df.rank[j])*10

                elseif df.teamA[j]=="BYE" 
                    costSelectBye[i,j] = abs.(size(df.rank,1)/2 .- df.rank[i])*10
                end

            elseif bye == :first 
                if df.teamA[i]=="BYE" 
                    costSelectBye[i,j] = df.rank[j]*10

                elseif df.teamA[j]=="BYE" 
                    costSelectBye[i,j] = df.rank[i]*10
                end
            ...
            elseif bye == :last 
                ...
                elseif df.teamA[j]=="BYE" 
                    costSelectBye[i,j] =  size(df.rank,1) +1 .- df.rank[i]*10
                end
            end
        end
    end
    ...
    prevGameGdf =combine(groupby(prevGameDf,[:teamA,:teamB]), :exclude => sum => :exclude)

    score_df = unstack(prevGameGdf, :teamA, :teamB, :exclude,fill = 0)
    ...
    @variable(model,possibleGames[1:teamSz,1:teamSz],Bin)

    # These constraints ensure that teams only play once. 
    @constraint(model, teamA[i = 1:teamSz], sum(possibleGames[i, j] for j in 1:teamSz) == 1)
    @constraint(model, teamB[j = 1:teamSz], sum(possibleGames[i, j] for i in 1:teamSz) == 1)

    # this ensures that both teams play each other, else it might try A => B and then B => C
    @constraint(model, parity[i = 1:teamSz, j = 1:teamSz], possibleGames[i, j] == possibleGames[j, i])
    ...
    schedule = DataFrame(teamA = String[], teamB = String[], rankA = Int[], rankB = Int[], streamA = Int[], streamB = Int[] )

    for i in pos
        if i[1] > i[2] # this will take half of the positions. ie, it will take [10,1] but not [1,10], which are the same
            push!(schedule,(df.teamA[i[1]], df.teamA[i[2]], df.rank[i[1]], df.rank[i[2]],df.streamed[i[1]] ,df.streamed[i[2]] ))
        end
    end
    ...
    schedule
    # ok, so now we need to take these pairings, and find out what field their previous games were on. 

    # get the current round
    latestRound = _swissDrawObject.currentRound

    if refresh == true 
        latestRound = _swissDrawObject.previousRound[end]
    end

    latestRoundDF = DataFrame(team = String[], fieldNumber = Int[])
    # if we are refreshing the rankings, we are not taking into account the games in the current draw.
        for i in latestRound.Games
            push!(latestRoundDF, (i.teamA,  i.fieldNumber ), promote=true)
            push!(latestRoundDF, (i.teamB,  i.fieldNumber ), promote=true)

        end

    leftjoin!(schedule,
        rename(latestRoundDF,:team => :teamA, :fieldNumber => :fieldNumberA),
        on = :teamA
    )

    leftjoin!(schedule,
        rename(latestRoundDF,:team => :teamB, :fieldNumber => :fieldNumberB),
        on = :teamB
    )

    schedule.key .= 1:size(schedule,1)

    # Need to make another distance matrix of teamPairings to field.
    fdf = deepcopy(_fieldLayout.fieldDF)
    
    sheduleSz = size(schedule,1)
    fieldSz = size(fdf,1)
    ...
    # order of priority:
    # 1 StreamFields get a game
    # 2 teams get a streamed game.
    # 3 teams walk less
    ...
    # @variable(model,possibleGames[1:teamSz,1:teamSz,1:fieldSz],Bin)
    @variable(model,gamesXfields[1:sheduleSz,1:fieldSz],Bin)

    # These constraints ensure that teams only play once. 
    @constraint(model, shedule[j = 1:fieldSz], sum(gamesXfields[i, j] for i in 1:sheduleSz) <= 1)
    @constraint(model, fields[i = 1:sheduleSz], sum(gamesXfields[i, j] for j in 1:fieldSz) == 1)

    # we want the minimum difference in matchups
    @objective(model,Min,sum(streamM .* gamesXfields ));
    model
    ...
    # find all the positions where 1 
    pos = findall(x->x==1,value.(gamesXfields))

    fieldAllocation = DataFrame(key = Int[], number = Int[])

    for i in pos
        push!(fieldAllocation,(i[1], i[2]))
    end
    ...
    # now we need to make games out of these
    g = Game[]

    for i in eachrow(schedule)
        push!(g,Game(i.teamA,i.teamB,i.number,missing,missing,i.stream))
    end

    NextRound = RoundOfGames(g,_SwissDraw.currentRound.roundNumber)
    # and push the current round to previous rounds if we are not refreshing the draw 
    if refresh == false
